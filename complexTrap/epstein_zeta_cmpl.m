function [S,Sd1,Sd2,Sd3,Sd4,Ssp2,Sd1sp2] = epstein_zeta_cmpl(s,E,F,G,L,M,N)
...
lambda = min(min(abs((E+G)/2 - sqrt((E-G).^2+4*F.^2)/2))); % min eigenvalue of Q
...
for i = 0:n-1     % right-half ij-plane & j-axis
    for j = 1:ceil(sqrt(n^2-i^2))
        ...
        if nargout > 1
            ...
            [g1,g1p1,g1p2,g1p3,g1p4] = incgamma_cmpl(s1,x);
            [g2,g2p1,g2p2,g2p3,g2p4] = incgamma_cmpl(s2,x);
            ...
            x = pi*QA(i,j);
            S = S+incgamma_cmpl(s1,x)+incgamma_cmpl(s2,x);
        end
        if nargout > 5
...
% gradient checking
ru = randn(3,1)+0.1*1i*randn(3,1); rv = randn(3,1)+0.1*1i*randn(3,1);
...
[~,     ~, Sd2_EG, Sd3_EpG, S4d_EpG] = epstein_zeta_cmpl(s,E,F,G,1,0,1);
[~,     ~,      ~, Sd3_EmG, S4d_EmG] = epstein_zeta_cmpl(s,E,F,G,1,0,-1);
[~, Sd1_E,  Sd2_E,   Sd3_E,   Sd4_E] = epstein_zeta_cmpl(s,E,F,G,1,0,0);
[S, Sd1_F,  Sd2_F,       ~,   Sd4_F] = epstein_zeta_cmpl(s,E,F,G,0,1,0);
[~, Sd1_G,  Sd2_G,   Sd3_G,   Sd4_G] = epstein_zeta_cmpl(s,E,F,G,0,0,1);
[~,     ~,      ~, Sd3_EpF] = epstein_zeta_cmpl(s,E,F,G,1,1,0);
[~,     ~,      ~, Sd3_EmF] = epstein_zeta_cmpl(s,E,F,G,1,-1,0);
...
epsi = 1e-4;
% (d/dF) and (d^2/dF^2)
S_Fp = epstein_zeta_cmpl(s,E,F+epsi,G,1,0,0);
S_Fm = epstein_zeta_cmpl(s,E,F-epsi,G,1,0,0);
Sd1_F_approx = (S_Fp - S_Fm)/(2*epsi);
Sd2_FF_approx = (S_Fp - 2*S + S_Fm)/epsi^2;
% (d^2/dEdG), (d^3/dE^2dG) and (d^3/dEdF^2)
[S_EpGp,Sd1_E_EpGp] = epstein_zeta_cmpl(s,E+epsi,F,G+epsi,1,0,0);
[S_EpGm,Sd1_E_EpGm] = epstein_zeta_cmpl(s,E+epsi,F,G-epsi,1,0,0);
[S_EmGp,Sd1_E_EmGp] = epstein_zeta_cmpl(s,E-epsi,F,G+epsi,1,0,0);
[S_EmGm,Sd1_E_EmGm] = epstein_zeta_cmpl(s,E-epsi,F,G-epsi,1,0,0);
[~,~,Sd2_E_Gp] = epstein_zeta_cmpl(s,E,F,G+epsi,1,0,0);
[~,~,Sd2_E_Gm] = epstein_zeta_cmpl(s,E,F,G-epsi,1,0,0);
[~,Sd1_G_Ep] = epstein_zeta_cmpl(s,E+epsi,F,G,0,0,1);
[~,Sd1_G_Em] = epstein_zeta_cmpl(s,E-epsi,F,G,0,0,1);
[~,Sd1_E_Fp] = epstein_zeta_cmpl(s,E,F+epsi,G,1,0,0);
[~,Sd1_E_Fm] = epstein_zeta_cmpl(s,E,F-epsi,G,1,0,0);
