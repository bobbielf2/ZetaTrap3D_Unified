function [Zs,Zsd1,Zsd2,Zsd3,Zsd4,Zsd5,Zsd6,Zsd7] = epstein_zeta_d7_cmpl(s,E,F,G)
...
ZdEF_mix = cell(N+1,1);
ZdFG_mix = cell(N+1,1);
[Zs,ZdEF_mix{1}] = epstein_zeta7_cmpl(s,E,F,G,1,0,0);
for k = 1:N     % NOTE: only N+1 func calls
    ak = cos(k*pi/2/N); bk = sin(k*pi/2/N);
    [~,ZdEF_mix{k+1}] = epstein_zeta7_cmpl(s,E,F,G,ak,bk/2,0);
    [~,ZdFG_mix{k+1}] = epstein_zeta7_cmpl(s,E,F,G,0,ak/2,bk);
end
...
s = -3;
[Zs,Zsd1,Zsd2,Zsd3,Zsd4,Zsd5,Zsd6,Zsd7] = epstein_zeta_d7_cmpl(s,E,F,G);
