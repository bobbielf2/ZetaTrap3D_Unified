function [gs,gsp] = incgamma10(s,x)
...
% Output:
%   gs       = igamma(s,x)*x^(-s)
%   gsp(k,:) = igamma(s+k,x)*x^(-(s+k)), k = 1,...,10
%
...
if numel(s) == 1 && s <= 0
    ...
    if nargout > 1
        if k >= 10
            gsp = gg(2:11,:);
        elseif k==0
            gsp = zeros(10,size(gg,2));
            gsp(1,:) = ex./x;
            for i = 1:9
                gsp(i+1,:) = ((s+i).*gsp(i,:)+ex)./x;
            end
        else
            gsp = [gg(2:k+1,:);zeros(10-k,size(gg,2))];
            for i = k:9
                gsp(i+1,:) = ((s+i).*gsp(i,:)+ex)./x;
            end
        end
...
for s = -5:0.5:5
    ...
    fprintf([']:\n\t',...
              ...
              'g(s+8,x):\tabs.err = %.1e \trel.err = %.1e\n\t',...
              'g(s+9,x):\tabs.err = %.1e \trel.err = %.1e\n\t',...
              'g(s+10,x):\tabs.err = %.1e \trel.err = %.1e\n'],...
              [err_abs;err_rel]);
end
end