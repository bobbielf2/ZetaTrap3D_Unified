function [zweis, indstens, stens] = Lap3dPointZeta_multi(ord,lptypes,P,inds)
% Compute zeta correction weights & their associated global column index in
% the matrix for the Laplace potentials on a curved-rectangular patch.
% Correction order <= 9 for weakly singular kernels & order <= 7 for
% hypersingular kernel

N = size(P.x,2);
if nargin < 4, inds = 1:N; end
...
for i = 1:np
    ...
    if strcmp(lptype,'s')       % SLP kernel 1/(4*pi)*r^(-1)*J
        cs{i} = {1/4/pi * P.J(indsten)};
    elseif strcmp(lptype,'d')   % DLP kernel 1/(4*pi)*r^(-3)*dot(ny,x-y)*J
        cs{i} = {1/4/pi * P.J(indsten) .* (P.NX{1}(indsten).*d{1}+P.NX{2}(indsten).*d{2}+P.NX{3}(indsten).*d{3})};
    elseif strcmp(lptype,'sn')  % SLPn kernel 1/(4*pi)*r^(-3)*dot(nx,y-x)*J
        cs{i} = {-1/4/pi * P.J(indsten) .* (P.nx(1,inds).*d{1}+P.nx(2,inds).*d{2}+P.nx(3,inds).*d{3})};
    elseif strcmp(lptype,'dn')  % DLPn kernel 1/(4*pi)*(r^(-3)*dot(nx,ny)-3*r^(-5)*dot(nx,x-y)*dot(ny,x-y))*J
        if ord <=1
            cs{i} = {1/4/pi * P.J(indsten)};
        elseif ord<=7
        cs{i} = {1/4/pi * P.J(indsten) .* (P.nx(1,inds).*P.NX{1}(indsten)+P.nx(2,inds).*P.NX{2}(indsten)+P.nx(3,inds).*P.NX{3}(indsten)),... % hypersingular part
            -3/4/pi * P.J(indsten) .* ...
                (P.NX{1}(indsten).*d{1}+P.NX{2}(indsten).*d{2}+P.NX{3}(indsten).*d{3}) ...
                 .*(P.nx(1,inds).*d{1}+P.nx(2,inds).*d{2}+P.nx(3,inds).*d{3})};  % weakly singular part
        end
    end
    r2mQh2s{i} = r2mQh2;
    indstens{i} = indsten;
...
elseif ord <= 3  % O(h^3) correction. DLPn need bigger stencil, others need 1-pt.
    if any(strcmp(lptypes,'dn'))
        ...
        [Zs,Zsd1]=epstein_zeta_d4_s2d1(1,E,F,G);
        pre.s = 1;
        pre.zeta={Zs,Zsd1};
    end
